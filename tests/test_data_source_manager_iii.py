#!/usr/bin/env python
"""Integration tests for DataSourceManager focusing on cache performance benchmarking.
...
def log_performance_metrics(
    operation: str, start_time: float, end_time: float, start_memory: float, end_memory: float, df: pd.DataFrame
) -> None:
...
async def perform_data_fetch(
    manager: DataSourceManager, start_time: datetime, end_time: datetime, use_cache: bool
) -> Tuple[pd.DataFrame, float, float, float, float]:
    """Perform data fetch and measure performance metrics."""
    start_memory = get_memory_usage()
    start_time_perf = time.perf_counter()

    df = await manager.get_data(symbol=TEST_SYMBOL, interval=TEST_INTERVAL, start_time=start_time, end_time=end_time, use_cache=use_cache)
...
async def test_basic_cache_performance(manager: DataSourceManager) -> None:
    ...
    logger.info("║   Measuring and comparing performance characteristics between cache hits and misses")
    logger.info("║   to understand the performance impact of caching in typical usage scenarios.")
    logger.info("║")
    ...
    df_cold, start_cold, end_cold, start_mem_cold, end_mem_cold = await perform_data_fetch(manager, start_time, end_time, use_cache=True)
    log_performance_metrics("Cold Cache Fetch", start_cold, end_cold, start_mem_cold, end_mem_cold, df_cold)

    # Warm cache fetch (cache hit)
    logger.info("\nPerforming warm cache fetch (cache hit)...")
    df_warm, start_warm, end_warm, start_mem_warm, end_mem_warm = await perform_data_fetch(manager, start_time, end_time, use_cache=True)
    log_performance_metrics("Warm Cache Fetch", start_warm, end_warm, start_mem_warm, end_mem_warm, df_warm)
    ...
    logger.info(f"║ • Cold Cache Memory Change: {end_mem_cold - start_mem_cold:+.2f} MB")
    logger.info(f"║ • Warm Cache Memory Change: {end_mem_warm - start_mem_warm:+.2f} MB")
    logger.info("╚════════════════════════════════════════════════════════════════════════════════════════════════════════")

    # Verify data consistency
    pd.testing.assert_frame_equal(df_cold, df_warm, check_dtype=True, check_index_type=True, check_column_type=True)
...
async def test_cache_vs_no_cache_comparison(manager: DataSourceManager) -> None:
    ...
    logger.info("╠════════════════════════════════════════════════════════════════════════════════════════════════════════")
    logger.info("║ 🎯 MOTIVATION:")
    logger.info("║   Directly comparing performance characteristics between cached and non-cached data retrieval")
    ...
    df_no_cache, start_no_cache, end_no_cache, start_mem_no_cache, end_mem_no_cache = await perform_data_fetch(
        manager, start_time, end_time, use_cache=False
    )
    log_performance_metrics("No-Cache Fetch", start_no_cache, end_no_cache, start_mem_no_cache, end_mem_no_cache, df_no_cache)

    # Cache fetch
    logger.info("\nPerforming cached fetch...")
    df_cache, start_cache, end_cache, start_mem_cache, end_mem_cache = await perform_data_fetch(
        manager, start_time, end_time, use_cache=True
    )
    log_performance_metrics("Cached Fetch", start_cache, end_cache, start_mem_cache, end_mem_cache, df_cache)
    ...
    logger.info("║")
    logger.info("║ Memory Impact:")
    logger.info(f"║ • No-Cache Memory Change: {end_mem_no_cache - start_mem_no_cache:+.2f} MB")
    logger.info(f"║ • Cache Memory Change: {end_mem_cache - start_mem_cache:+.2f} MB")
    logger.info("╚════════════════════════════════════════════════════════════════════════════════════════════════════════")

    # Verify data consistency
    pd.testing.assert_frame_equal(df_no_cache, df_cache, check_dtype=True, check_index_type=True, check_column_type=True)
...
async def test_geometric_range_performance(manager: DataSourceManager) -> None:
    ...
    logger.info("╠════════════════════════════════════════════════════════════════════════════════════════════════════════")
    logger.info("║ 🎯 MOTIVATION:")
    logger.info("║   Analyzing how cache performance scales with geometrically increasing data ranges")
    logger.info("║   to understand the relationship between data size and performance benefits.")
    logger.info("║")
    logger.info("║ 📋 TEST SEQUENCE:")
    ...
    for minutes in ranges:
        ...
        df_cold, start_cold, end_cold, start_mem_cold, end_mem_cold = await perform_data_fetch(
            manager, start_time, end_time, use_cache=True
        )
        cold_duration = end_cold - start_cold
        cold_memory = end_mem_cold - start_mem_cold
        log_performance_metrics(f"Cold Cache Fetch ({minutes}min)", start_cold, end_cold, start_mem_cold, end_mem_cold, df_cold)

        # Warm cache fetch
        logger.info(f"Performing warm cache fetch for {minutes}-minute range...")
        df_warm, start_warm, end_warm, start_mem_warm, end_mem_warm = await perform_data_fetch(
            manager, start_time, end_time, use_cache=True
        )
        warm_duration = end_warm - start_warm
        warm_memory = end_mem_warm - start_mem_warm
        log_performance_metrics(f"Warm Cache Fetch ({minutes}min)", start_warm, end_warm, start_mem_warm, end_mem_warm, df_warm)
        ...
        results.append(
            {
                ...
                "speedup": cold_duration / warm_duration if warm_duration > 0 else float("inf"),
                "memory_efficiency": cold_memory / warm_memory if warm_memory > 0 else float("inf"),
            }
        )
    ...
    logger.info("╠════════════════════════════════════════════════════════════════════════════════════════════════════════")
    logger.info("║ Range  Records  Cold(s)  Warm(s)  Speedup  Cold Mem(MB)  Warm Mem(MB)  Mem.Eff")
    logger.info("║ ─────────────────────────────────────────────────────────────────────────────")
    ...
    logger.info(f"║ • Memory Efficiency Trend: {', '.join(f'{m:.2f}x' for m in mem_effs)}")
    logger.info("╚════════════════════════════════════════════════════════════════════════════════")
