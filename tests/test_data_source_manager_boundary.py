...
async def test_boundary_conditions(manager: DataSourceManager):
    ...
    for case in test_cases:
        ...
        logger.info(f"  - First record should be >= {start_time.replace(microsecond=0).isoformat()}")
        logger.info(f"  - Last record should be <= {end_time.replace(microsecond=0).isoformat()}")  # Updated to <= instead of <

        try:
            # Execute data fetch
            if case["case_number"] == 4:  # Zero-duration request
                with pytest.raises(ValueError, match="Start date must be before end date"):
                    df = await manager.get_data(symbol=TEST_SYMBOL, start_time=start_time, end_time=end_time, use_cache=False)
            ...
            logger.info(f"Original:  {start_time.isoformat()} -> {end_time.isoformat()}")
            if not df.empty:
                ...
                logger.info(f"Actual:    {first_time.isoformat()} -> {last_time.isoformat()}")
            else:
            ...
            validation = {
                ...
                "Record count": len(df),
                "Actual duration": df.index[-1] - df.index[0] if len(df) > 1 else timedelta(0),
                "Status": "PASSED" if len(df) == case["expected_records"] else "FAILED",
            }

            # Additional boundary checks
            if not df.empty:
                validation["First >= start"] = df.index[0] >= start_time.replace(microsecond=0)
                validation["Last <= end"] = df.index[-1] <= end_time.replace(microsecond=0)  # Updated to <= instead of <
            ...
            if not df.empty:
                assert df.index[0] >= start_time.replace(microsecond=0), "First timestamp too early"
                assert df.index[-1] <= end_time.replace(microsecond=0), "Last timestamp too late"  # Updated to <= instead of <
                assert df.index[-1] < datetime.now(timezone.utc), "Contains future data"

        except Exception as e:
            if case["case_number"] == 4 and isinstance(e, ValueError) and str(e) == "Start date must be before end date":
                logger.info("Zero-duration request correctly raised ValueError")
            else:
