...
parser = argparse.ArgumentParser(
    description="Test Binance Vision data availability (curl_cffi version)"
)
parser.add_argument(
    "--market",
    default="spot",
...
async def check_url_availability(url, timeout, session, debug=False):
    ...
    for i in range(max_days_back + 1):
        ...
        for retry in range(max_retries):
            ...
            is_available, status_code, elapsed = await check_url_availability(
                url, timeout, session, debug=debug
            )
            ...
            # If first try with HEAD failed, try GET
            if retry == 0 and (status_code == 404 or status_code is None):
                try:
                    start_time = time.time()
...
async def main():
    ...
    async with curl_requests.AsyncSession() as session:
        ...
        if args.direct_url:
            if args.debug:
                print(f"Checking URL: {args.direct_url}")
                print(f"Timeout: {args.timeout} seconds")
                print(f"Max retries: {args.max_retries}")

            # Try up to max_retries times with increasing backoff
            for retry in range(args.max_retries):
                if retry > 0 and args.debug:
                    print(f"Retry #{retry}...")

                is_available, status_code, elapsed = await check_url_availability(
                    args.direct_url, args.timeout, session, debug=args.debug
                )

                if is_available:
                    print(f"URL is available (verified with HEAD request)")
                    return
                ...
                if retry == 0 and (status_code == 404 or status_code is None):
                    try:
                        ...
                        response = await session.get(
                            args.direct_url, timeout=args.timeout
                        )
                        status_code = response.status_code
                        success = status_code == 200
                        ...
                        if success:
                            print(f"URL is available (verified with GET request)")
                            return
                    except Exception as e:
                        if args.debug:
                            print(f"GET Error: {e}")
